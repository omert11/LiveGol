...
class BankoRec(val kuponlar: ArrayList<Kupondata>, val Contex: Context, val itemClickListener: (Int,String) -> Unit) : RecyclerView.Adapter<BankoRec.Kasaviewholder>() {
    ...
    inner class Kasaviewholder(val Viev: View, var ViewGroup: ViewGroup) : androidx.recyclerview.widget.RecyclerView.ViewHolder(Viev) {
        ...
        fun atama(position: Int) {
            ...
            FirebaseDatabase.getInstance().reference.child("Yorumlar/Banko/" + position.toString()).addListenerForSingleValueEvent(object : ValueEventListener {
        ...
        private fun yorumyolla(position:Int) {
            ...
            if (yorumet.text.isEmpty()){
                ...
                FirebaseDatabase.getInstance().reference.child("Yorumlar/Banko/" + position.toString()+"/Yorum/"+yorumsayisi).setValue(yorumdata(Kullanicidata.Res!!, Kullanicidata.Reslow!!, Kullanicidata.Nick!!,yorumet.text.toString())).addOnCompleteListener {
                    if (it.isComplete){
        ...
        private fun begenbutonyapilacak(position: Int) {
            if (!begendimmi) {
                FirebaseDatabase.getInstance().reference.child("Yorumlar/Banko/" + position.toString()).child("Begenme").child(Uid_kullanan).ref.setValue(Uid_kullanan)
            } else {
                FirebaseDatabase.getInstance().reference.child("Yorumlar/Banko/" + position.toString()).child("Begenme").child(Uid_kullanan).ref.removeValue()
        }
    }
        fun <T : TextView> T.onClick(event:(Pos: (Int),Kuponismi:String) -> Unit): T {
            yorumtamminigor.setOnClickListener {
                event.invoke(POs,"Banko")
            }
            return this
        }
    }
